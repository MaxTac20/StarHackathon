...
A production-capable, deliberately small starter for hackathons, MVPs, and small-to-medium applications. It combines a FastAPI API, React client, and PostgreSQL without adding layers that a new product does not yet need.
...
Open <http://localhost:5173>. Vite proxies `/api` to FastAPI at port 8000. API docs are at <http://localhost:8000/docs> in development.
...
Open <http://localhost:5173>. Frontend, backend, and PostgreSQL run separately with source bind mounts and hot reload. The development database is bound only to `127.0.0.1:5432`.
...
Open <http://localhost:8000> and <http://localhost:8000/example>. Compose first runs the one-shot Alembic migration service, then starts the application. PostgreSQL has no host port in production Compose.
